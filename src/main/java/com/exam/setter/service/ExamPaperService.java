...
public class ExamPaperService {
    ...
    public ExamPaperEntity assembleAndPersistExamPaper(ExamPaperBlueprintRequest request) {
        ...
        for (SectionBlueprint section : request.sections()) {
            ...
            List<QuestionEntity> questionEntities = generatedQuestions.stream().map(gq -> QuestionEntity.builder().section(sectionEntity).questionText(gq.questionText()).questionType(gq.questionType()).options(gq.options()).correctAnswer(gq.correctAnswer()).explanation(gq.explanation()).difficulty(gq.difficulty()).marks(gq.marks()).topic(gq.topic()).visualRequired(gq.visualRequired()).visualType(gq.visualType()).visualDescription(gq.visualDescription()).sourceCitationsJson(writeCitations(gq.sourceCitations())).moderationStatus(ModerationStatus.PENDING_REVIEW).includedInPaper(false).build()).toList();
    ...
    public AssembledExamPaper assembleExamPaper(ExamPaperBlueprintRequest request) { validateBlueprint(request); int totalMarks = 0, totalQuestions = 0; List<AssembledExamPaper.AssembledSection> sections = new ArrayList<>(); for (SectionBlueprint section : request.sections()) { QuestionGenerationRequest genRequest = new QuestionGenerationRequest(request.subject(), request.targetLevels(), section.questionType(), section.difficulty(), section.questionCount(), section.marksPerQuestion(), request.examId(), section.topic(), request.knowledgeSource(), request.knowledgeSources(), request.corpusVersion(), request.ncertBookCode(), request.ncertChapterNumber()); List<GeneratedQuestion> questions = generate(genRequest); int marks = questions.size() * section.marksPerQuestion(); totalMarks += marks; totalQuestions += questions.size(); sections.add(new AssembledExamPaper.AssembledSection(section.sectionName(), marks, section.negativeMarks(), questions)); } return new AssembledExamPaper(request.examTitle(), request.subject().trim().toLowerCase(), request.targetLevels(), request.durationMinutes(), totalMarks, totalQuestions, sections, Instant.now()); }
}
